...
docker compose -p client $COMPOSE_FILES pull || log WARN "Pull failed or skipped for dev"

docker compose -p client $COMPOSE_FILES build --pull || {
    log ERROR "Docker build failed"
    exit 1
}

docker compose -p client $COMPOSE_FILES up -d || {
    log ERROR "Compose up failed"
    exit 1
}

docker compose -p client $COMPOSE_FILES ps || log WARN "Unable to check container status"
