...
if [ "$CLIENT_LOCAL_ENCODER" = "h264_v4l2m2m" ]; then
	ffmpeg \
	...
	ffmpeg \
		...
		-vf "fps=30,drawtext=fontfile=/usr/share/fonts/dejavu/DejaVuSans.ttf:text='%{localtime} UTC':x=W-tw-10:y=10:fontcolor=white:box=1:boxcolor=0x00000099" \
		-hls_time 5 \
		-hls_list_size 17280 \
		-hls_segment_filename "/archive/${START_DATE}_%05d.ts" \
		-hls_flags program_date_time+append_list+omit_endlist+independent_segments+delete_segments \
